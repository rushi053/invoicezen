...
function ItemsTable({ invoice, cur, accent, bold }: { invoice: InvoiceData; cur: { symbol: string }; accent?: boolean; bold?: boolean }) {
...
function SummaryBlock({ invoice, cur, subtotal, total, accent, bold }: TplProps & { accent?: boolean; bold?: boolean }) {
  const totalColor = accent ? "text-emerald-600" : "text-gray-900";
  return (
    <div className="w-56 text-xs space-y-1">
      ...
      <div className={`flex justify-between font-${bold ? "black" : "bold"} text-base border-t border-gray-200 pt-2`}>
        <span>Total</span><span className={totalColor}>{cur.symbol}{total.toFixed(2)}</span>
      </div>
    </div>
  );
}
